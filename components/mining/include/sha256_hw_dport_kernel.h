 ...
 *   1. Fill TEXT[0..15] = block 1 (header[0..63]).
 ...
 *   8. Wait BUSY=0; SHA_LOAD final digest → TEXT[0..7].
 *   9. Wait BUSY=0; raw read of TEXT[7] for early-reject vs target_word0_max.
 *  10. Potential hit: DPORT_INTERRUPT_DISABLE + DPORT_SEQUENCE_REG_READ of all
                            ...
                            uint8_t hash_out[32])
{
    ...
    *sha_start = 1;
    ...
    /* 8. Wait block-3, LOAD final digest */
    while (*sha_busy) {}
    *sha_load = 1;

    /* 9. Wait LOAD; raw early-reject on word 7 (no DPORT_SEQ needed —
    ...
    /* 10. Full readback under DPORT erratum workaround */
